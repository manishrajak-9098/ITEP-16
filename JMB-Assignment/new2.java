        ...
        switch (choice) {
            ...
            case 2:
                ...
                if (n2 == 1) {
                    ...
                    breakfastPrice = 200; // Correctly set price
                } else if (n2 == 2) {
                    breakfastname = " BreakFast 'B' -> (rs 230) ";
                    breakfastPrice = 230; // Correctly set price
                } else if (n2 == 3) { // Corrected this condition from 'n2 == 5'
                    breakfastname = " BreakFast 'C' -> (rs 250) ";
                    breakfastPrice = 250; // Correctly set price
                } else {
                    System.out.println(RED_TEXT + "Invalid Choice !" + RESET);
                }
                ...
                // This if-else if block is redundant now as prices are set above, but doesn't hurt.
                // It should mirror the block above it. I'll remove it for clarity.
                /*
                if(n2==1){
                    breakfastPrice = 200;
                }
                ...
                else if (n2 == 3){ // Corrected from n2 == 5
                    breakfastPrice = 250;
                }
                else {
...
//for Ladies...............
            case 3:
                ...
                if (n3 == 1) {
                    ...
                    ladiesdishprice = 250; // Correctly set price
                } else if (n3 == 2) {
                    ladiesdish = " LadiesMenu 'B' -> (rs 300) ";
                    ladiesdishprice = 300; // Correctly set price
                } else if (n3 == 3) {
                    ladiesdish = " LadiesMenu 'C' -> (rs 350) ";
                    ladiesdishprice = 350; // Correctly set price
                } else {
                    System.out.println(RED_TEXT + "Invalid Choice !" + RESET);
                }
...
//====================================================Formula od Bill Condition============================================
        ...
        // int extraChargePerHead =0; // This variable is declared but never used. Removed for clarity.
        int total = 0;
        double gst = 0;
        double totalBill = 0;

        // Corrected calculation logic for minAllowed/maxAllowed based on 100% and 110% as per previous interpretation.
        // It now charges for actual attendees if within 100-110% of booked, or booked + extra if over 110%,
        // and always at least for booked if fewer attend.
        if (choice == 1) { // Vegetarian Menu (Thali)

            int minToConsiderBooked = Quantity1; // Minimum guests to be considered for booked price
            int maxAllowedWithoutExtraCharge = (int) (Quantity1 * 1.10); // 110% of booked

            if (Quantity2 >= minToConsiderBooked && Quantity2 <= maxAllowedWithoutExtraCharge) {
                // If actual guests are between 100% and 110% of booked, charge for booked quantity
                total = Quantity1 * thaliPrice;
            } else if (Quantity2 > maxAllowedWithoutExtraCharge) {
                // If actual guests exceed 110% of booked, charge for booked + extra exceeding 110%
                int guestsToChargeFor = Quantity1 + (Quantity2 - maxAllowedWithoutExtraCharge);
                total = guestsToChargeFor * thaliPrice;
            } else { // Quantity2 < minToConsiderBooked (i.e., less than booked quantity)
                // If fewer than booked attend, still charge for the booked quantity
                total = Quantity1 * thaliPrice;
            }
            gst = total * 0.18;
            totalBill = total + gst;
...
//=====================================================================================
            ...
            int minToConsiderBooked = Quantity1;
            int maxAllowedWithoutExtraCharge = (int) (Quantity1 * 1.10);

            if (Quantity2 >= minToConsiderBooked && Quantity2 <= maxAllowedWithoutExtraCharge) {
                total = Quantity1 * breakfastPrice;
            } else if (Quantity2 > maxAllowedWithoutExtraCharge) {
                int guestsToChargeFor = Quantity1 + (Quantity2 - maxAllowedWithoutExtraCharge);
                total = guestsToChargeFor * breakfastPrice;
            } else {
                total = Quantity1 * breakfastPrice;
            }
            gst = total * 0.18;
...
//==================================================================================
            ...
            if (Quantity2 >= minToConsiderBooked && Quantity2 <= maxAllowedWithoutExtraCharge) {
                ...
                int guestsToChargeFor = Quantity1 + (Quantity2 - maxAllowedWithoutExtraCharge);
                total = guestsToChargeFor * ladiesdishprice;
            } else {
                total = Quantity1 * ladiesdishprice;
            }
            gst = total * 0.18;
        ...
        while (true) {
            ...
            int bv;
            while (true) { // Input validation for bv
                if (sc.hasNextInt()) {
                    bv = sc.nextInt();
                    sc.nextLine(); // Consume newline
                    if (bv >= 1 && bv <= 3) {
            ...
            switch (bv) {
                case 1:
                    ...
                    System.out.printf(WHITE_BACKGROUND + RED_TEXT + "%-60s" + RESET + "\n", "Quantity                        : " + Quantity2);
                    System.out.printf(WHITE_BACKGROUND + RED_TEXT + "%-60s" + RESET + "\n", "----------------------------------------------------");
                    System.out.printf(WHITE_BACKGROUND + RED_TEXT + "%-60s" + RESET + "\n", "BreakFast Details               : \n" + breakfast); // Displaying full breakfast string
                    System.out.printf(WHITE_BACKGROUND + RED_TEXT + "%-60s" + RESET + "\n", "----------------------------------------------------");
                    System.out.printf(WHITE_BACKGROUND + RED_TEXT + "%-60s" + RESET + "\n", "Total Amount                    : Rs " + total);
                    System.out.printf(WHITE_BACKGROUND + RED_TEXT + "%-60s" + RESET + "\n", "Total Bill Amount (Gst)          : Rs " + String.format("%.2f", totalBill));
                    System.out.printf(WHITE_BACKGROUND + RED_TEXT + "%-60s" + RESET + "\n", "====================================================");
                ...
                case 2:
                    ...
                    while (true) {   // sirf thali ke liye loop
                        // Input validation for confirm_no
                        if (sc.hasNextInt()) {
                            confirm_no = sc.nextInt();
                            sc.nextLine(); // Consume newline
                            if (confirm_no >= 1 && confirm_no <= 5) {
                                break;
                            } else {
                                System.out.println(RED_TEXT + "Invalid Thali number. Please choose between 1 and 5." + RESET);
                            }
                ...
                case 3:
                    System.out.println("Plzz Confirm Which Type Of Ladies Menu You Book ! "); // Changed from Thali
                    System.out.println("Press 1 for 'A' / 2 For  'B'  / 3 For  'C'  ");

                    int confirm_no2;
                    while (true) {   // sirf thali ke liye loop
                        // Input validation for confirm_no2
                        if (sc.hasNextInt()) {
                            confirm_no2 = sc.nextInt();
                            sc.nextLine(); // Consume newline
                            if (confirm_no2 >= 1 && confirm_no2 <= 3) {
                                break;
                            } else {
                                System.out.println(RED_TEXT + "Invalid Ladies Menu number. Please choose between 1 and 3." + RESET);
                            }
                ...
                default:
                    System.out.println(RED_TEXT + " Invalid Input Plzz ReChoose" + RESET);
